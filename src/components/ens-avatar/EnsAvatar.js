...
import * as styles from "./ens-avatar.module.scss"

export default function EnsAvatar({ image, name, defaultImg, forceDefault }) {
	return (
		<div className={styles.card}>
			{name != null && !forceDefault ? (
				<svg
					width="270"
					height="270"
					viewBox="0 0 270 270"
					...
					style={{
						...
						width: "300px",
						height: "300px",
						backgroundImage: "linear-gradient(150deg, rgba(120,143,247,1) 0%, rgba(67,187,240,1) 100%)",
						backgroundSize: "contain",
					}}
				>
					<defs>
						<pattern id="backImg" patternUnits="userSpaceOnUse" x="0" y="0" width="270" height="270">
							<image href={image} width="270" height="270"></image>
						</pattern>
						<filter id="shadowy">
							<feDiffuseLighting in="SourceGraphic" result="light" lightingColor="white">
								<feDistantLight azimuth="240" elevation="40"></feDistantLight>
							</feDiffuseLighting>
							<feComposite in="SourceGraphic" in2="light" operator="arithmetic" k1="1" k2="0" k3="0" k4="0"></feComposite>
						</filter>
					</defs>
					<rect width="270" height="270" fill="url(#backImg)" filter="url(#shadowy)"></rect>
					<defs>
						<filter id="dropShadow" colorInterpolationFilters="sRGB" filterUnits="userSpaceOnUse" height="270" width="270">
							<feDropShadow dx="0" dy="1" stdDeviation="2" floodOpacity="0.225" width="200%" height="200%"></feDropShadow>
						</filter>
					</defs>
					<path
					...
					<path
						...
						fill="white"
					></path>
					<text x="32.5" y="231" fontSize="32px" fill="white" filter="url(#dropShadow)">
						{name}
					</text>
					<defs>
				...
				<img className={styles.defaultImg} width="300" height="300" src={defaultImg}></img>
			)}
		</div>
	)
}
